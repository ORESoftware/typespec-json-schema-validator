...
test('salvaged five-runtime manifest is valid against the published Draft 2020-12 contract', async () => {
  const manifest = await loadManifest();
...
test('five distinct required runtimes pass only with exact TJSV parity, Contract IR, and evidence bindings', async () => {
...
test('missing one required runtime receipt cannot be hidden by the other four passing runtimes', async () => {
  const input = await validInput();
...
test('symbolic source labels are never accepted as immutable cross-runtime evidence', async () => {
  const input = await validInput();
