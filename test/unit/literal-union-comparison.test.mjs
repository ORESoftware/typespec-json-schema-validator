...
test('literal-union and simple closed-object comparison rules compose without touching executable schemas', () => {
  const generated = {
    type: 'object',
    properties: {
      direction: {
  ...
  assert.equal(normalized(generated), normalized(authored));
  assert.ok(Object.hasOwn(normalizeSchemaNode(generated), 'unevaluatedProperties'));
  assert.ok(Object.hasOwn(normalizeSchemaNode(generated).properties.direction, 'anyOf'));
});
