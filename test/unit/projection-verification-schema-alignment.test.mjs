...
test('projection policy schema publishes the runtime identifier and media-type languages exactly', async () => {
...
test('projection policy schema publishes the runtime cardinality limits exactly', async () => {
  const value = await schema('projection-verification-policy.schema.json');
  assert.equal(value.properties.toolchains.maxItems, MAX_TOOLCHAINS);
  assert.equal(value.properties.requiredProjections.maxItems, 256);
...
test('projection receipt schema publishes the runtime finding identifier and bound exactly', async () => {
...
test('published path grammar rejects every path rejected by the runtime boundary examples', async () => {
  const value = await schema('projection-verification-policy.schema.json');
  const pattern = new RegExp(value.$defs.relativePath.pattern, 'u');
  const accepted = [
    'operations/api-ir.json',
    'generated/protobuf/accounts.proto',
    'a',
  ];
  const rejected = [
    '',
