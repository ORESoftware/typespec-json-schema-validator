...
test('missing or failed evidence status never becomes an implicit pass', () => {
  ...
  const result = verifyLanguageBoundaries({
    ...
    evidenceByPath: new Map([['rust/native.json', {
      schema: 'ores.typespec-json-schema-validator.language-boundary-evidence/v1', language: 'rust', runtime: 'native',
  ...
  assert.ok(result.findings.some((entry) => entry.ruleId === 'boundary-evidence-not-passed'));
});
