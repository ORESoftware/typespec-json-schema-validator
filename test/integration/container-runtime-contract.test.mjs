...
const fixture = resolve(packageRoot, 'test/fixtures/container-runtime-contract');
...
test('container runtime peer authorities emit Contract IR only after exact parity', async () => {
  const temp = await mkdtemp(join(tmpdir(), 'tsjsv-container-runtime-'));
  ...
  assert.equal(report.differential.summary.corpusInstances, 3);
  ...
  assert.equal(contractIr.declarations.length, 4);
});
