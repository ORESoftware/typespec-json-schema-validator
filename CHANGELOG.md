...
- Scheduled `npm-advisory-exception-expiry` workflow that runs the same gate daily and reconciles a single tracking issue, identified by the `tjsv-npm-advisory-exception-expiry` label plus a hidden body marker: created or updated (never duplicated) while any exception is expired or inside the warning window, closed with a resolution comment naming the run and commit once a passing run is clean, and reopened if an alert arises again.
