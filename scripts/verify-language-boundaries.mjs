...
async function declaredEvidence(workspace, evidenceRoot, manifest) {
  if (!Array.isArray(manifest?.targets)) throw new Error('language-boundary manifest targets must be an array');
  ...
  for (const target of manifest.targets) {
    ...
    if (seen.has(evidence)) throw new Error('language-boundary evidence paths must be unique');
    seen.add(evidence);
...
async function outputPath(workspace, value, fallback) {
  ...
  if (typeof selected !== 'string' || selected.trim() === '') throw new Error('missing TSJSV_BOUNDARY_VERIFICATION');
