...
function normalizePathDescriptor(value, label) {
  ...
  assertCondition(
    ...
    `${label}.path must be a normalized relative POSIX path`,
  );
...
function normalizeOutputDescriptor(value, label) {
  ...
  assertCondition(
    validIdentifier(value.projection),
    `${label}.projection must be a bounded lowercase identifier`,
  );
...
function normalizeToolchain(value, label) {
  ...
  assertCondition(
    ...
    `${label}.artifactDigest must be a lowercase SHA-256 digest`,
  );
  return Object.freeze({
    id: value.id,
...
function normalizeIdentifierArray(value, label, limit, { requireNonEmpty = false } = {}) {
  normalizeArray(value, label, limit);
...
function normalizeDeclarationArray(value) {
  const label = 'projection verification policy expectedDeclarations';
  normalizeArray(value, label, MAX_DECLARATIONS);
  assertCondition(value.length > 0, `${label} must not be empty`);
  const seen = new Set();
  const normalized = [];
  for (let index = 0; index < value.length; index += 1) {
    const item = value[index];
    assertCondition(validDeclarationId(item), `${label}[${index}] must be a bounded declaration identity`);
    assertCondition(!seen.has(item), `${label} must not contain duplicate identities`);
    seen.add(item);
    normalized.push(item);
  }
  return Object.freeze(normalized.sort((left, right) => left.localeCompare(right)));
...
function canonicalObject(value, label) {
  ...
  try {
    return Object.freeze(JSON.parse(canonicalStringify(value)));
  } catch {
...
export function normalizeProjectionVerificationPolicy(value) {
  ...
  assertCondition(
    ...
    'projection verification input paths must be unique',
  );

  const toolchains = normalizeArray(
    value.toolchains,
    'projection verification policy toolchains',
    MAX_TOOLCHAINS,
  )
    .map((item, index) => normalizeToolchain(
      item,
      `projection verification policy toolchains[${index}]`,
    ))
  ...
  const outputs = normalizeArray(
    value.outputs,
    'projection verification policy outputs',
    MAX_OUTPUTS,
  )
    .map((item, index) => normalizeOutputDescriptor(
      item,
      `projection verification policy outputs[${index}]`,
    ))
  ...
  const allPaths = [...inputPaths, ...outputs.map((item) => item.path)];
  assertCondition(
    new Set(allPaths).size === allPaths.length,
    'projection verification inputs and outputs must not reuse a path',
  );
  ...
  const approvedDeltas = normalizeArray(
    ...
    .map((item, index) => canonicalObject(
      item,
      `projection verification policy approvedDeltas[${index}]`,
    ))
  ...
  const runtimeValidators = normalizeArray(
    ...
    .map((item, index) => canonicalObject(
      item,
      `projection verification policy runtimeValidators[${index}]`,
    ))
    .sort((left, right) => canonicalStringify(left).localeCompare(canonicalStringify(right)));

  return Object.freeze({
    schema: PROJECTION_VERIFICATION_POLICY_SCHEMA,
    expectedDeclarations: normalizeDeclarationArray(value.expectedDeclarations),
    inputs,
...
export async function loadProjectionVerificationPolicy(path, options = {}) {
  try {
    const parsed = await loadProjectionManifest(path, {
      maxBytes: options.maxBytes ?? MAX_POLICY_BYTES,
    });
    return normalizeProjectionVerificationPolicy(parsed);
  } catch (error) {
    if (error instanceof ProjectionVerificationPolicyError) throw error;
    throw new ProjectionVerificationPolicyError('projection verification policy could not be read safely');
  }
}
